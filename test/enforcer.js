...
describe('enforcer', () => {
    ...
    describe('defaults', () => {
        const options = schemas.response.normalize({ useDefaults: true });

        it('valid string', () => {
            const schema = { default: 'abc' };
            const v = enforce(schema, options);
            expect(v).to.equal('abc');
        });

        it('invalid string', () => {
            const schema = { type: 'string', default: 1 };
            expect(code(() => enforce(schema, options))).to.equal('ESRTYPE');
        });
        ...
        it('invalid object via property', () => {
            const schema = {
                type: 'object',
                properties: {
                    foo: {
            ...
            expect(code(() => enforce(schema, options))).to.equal('ESRENUM');
        });

        it('valid object via property', () => {
            const schema = {
                type: 'object',
                properties: {
                    foo: {
        ...
        describe('mixed nested defaults', () => {
            ...
            it('can add partial object', () => {
                const o = { name: 'Bob' };
                const proxy = enforce(schema, options, o);
